...
import 'package:nde_email/presantation/mail/mail_detail/mail_image_viewer.dart';
...
class _AttachmentWidgetState extends State<AttachmentWidget> {
  ...
  static const String _androidLegacyMailMediaDir =
      "/storage/emulated/0/Android/media/$_androidPackageName/NowDigitalEasy/Mail/Media";
  static const String _androidLegacyMediaDir =
      "/storage/emulated/0/Android/media/$_androidPackageName/NowDigitalEasy/Media";
  static const String _mailMediaSubDir = "NowDigitalEasy/Mail/Media";

  Future<Directory> _ensureDownloadDirectory() async {
    final base = await getApplicationDocumentsDirectory();
    final directory = Directory(p.join(base.path, _mailMediaSubDir));
  ...
  bool _looksLikePdf(Uint8List data) =>
      _startsWithBytes(data, [0x25, 0x50, 0x44, 0x46]);
  bool _looksLikeJpeg(Uint8List data) =>
      _startsWithBytes(data, [0xFF, 0xD8, 0xFF]);
  ...
  bool _looksLikeGif(Uint8List data) =>
      _startsWithBytes(data, [0x47, 0x49, 0x46, 0x38]);
  ...
  Uint8List _extractAttachmentBytes({
    ...
    final expectedType =
        headerContentType.isNotEmpty ? headerContentType : fallbackContentType;
    final raw = Uint8List.fromList(response.bodyBytes);

    if (headerContentType.contains('application/json') ||
        _looksLikeJsonBytes(raw)) {
      try {
  ...
  Future<void> _openOrDownloadAttachment(
      {required bool forceRedownload}) async {
    try {
      ...
      var isImage =
          _isImageType(fileName: savedFileName, contentType: contentType);

      var exists = await file.exists();
      var length = exists ? await file.length() : 0;

      if ((!exists || length == 0) && Platform.isAndroid) {
        final legacyCandidates = <String>[
          _androidLegacyMailMediaDir,
          _androidLegacyMediaDir,
        ];

        for (final legacyDir in legacyCandidates) {
          final legacyPath = p.join(legacyDir, savedFileName);
      ...
      if (exists && length > 0 && !forceRedownload) {
        if (isImage) {
          ...
          await _openFile(file, contentType);
        }
        return;
      }
      ...
      if (response.statusCode == 200) {
        ...
        // Detect extension from bytes if still missing
        if (p.extension(file.path).isEmpty) {
          String? ext;
        ...
        isImage = _isImageType(
          ...
          contentType: responseContentType.isNotEmpty
              ? responseContentType
              : contentType,
        );
        ...
        Messenger.alertSuccess('Saved to: ${p.basename(file.path)}');

        if (isImage) {
          _showImagePreview(file);
        } else {
          final openType = responseContentType.isNotEmpty
              ? responseContentType
              : contentType;
          await _openFile(file, openType);
        }
      } else {
  ...
  Future<void> _openFile(File file, String? type) async {
    final cleanType = (type ?? '').contains('octet-stream') ? null : type;

    // Attempt 1: With Mime Type
    var result = await OpenFilex.open(file.path, type: cleanType);

    // Attempt 2: If failed, try without Mime Type (guess from extension)
    if (result.type != ResultType.done && cleanType != null) {
      result = await OpenFilex.open(file.path);
    }

    // Attempt 3: If still failed and it's a PDF, try explicit application/pdf
    if (result.type != ResultType.done &&
        p.extension(file.path).toLowerCase() == '.pdf') {
      result = await OpenFilex.open(file.path, type: 'application/pdf');
    }
    ...
    if (result.type != ResultType.done) {
      ...
      if (msg.contains('permission denied') ||
        ...
        try {
          ...
          final tmpPath = p.join(tmpDir.path, p.basename(file.path));
          final tmpFile = File(tmpPath);
          if (!await tmpFile.exists() || await tmpFile.length() == 0) {
            await file.copy(tmpPath);
          }
          result = await OpenFilex.open(tmpPath, type: cleanType);
          if (result.type != ResultType.done && cleanType != null) {
            result = await OpenFilex.open(tmpPath);
          }
        } catch (_) {}
      }
    }
    ...
    if (result.type != ResultType.done) {
      Messenger.alertError(
          "Could not open ${p.basename(file.path)}: ${result.message}");
    }
  }
  ...
  void _showImagePreview(File file) {
    if (context.mounted) {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => MailImageViewer(
            file: file,
            fileName: _buildDisplayFileName(),
          ),
        ),
      );
    }
